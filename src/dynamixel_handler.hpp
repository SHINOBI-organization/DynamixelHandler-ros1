...
class DynamixelHandler {
    ...
    static void Initialize(string device_name, int baudrate, bool varbose, int id_max ,int loop_rate, int error_ratio) {   
        ...
        for (auto id : id_list_x_) {
            dynamixel_chain[id].present_position = dyn_comm_.Read(id, present_position); // エラー時は0
            dynamixel_chain[id].goal_position    = dyn_comm_.Read(id, goal_position);    // エラー時は0
        }
        ...
        for (auto id : id_list_x_) {
            ...
            dyn_comm_.Write(id, operating_mode, OPERATING_MODE_EXTENDED_POSITION);  
            dyn_comm_.Write(id, profile_acceleration, 500); // 0~32767 数字は適当
            dyn_comm_.Write(id, profile_velocity, 100); // 0~32767 数字は適当
            int present_pos = dyn_comm_.Read(id, present_position);
            dyn_comm_.Write(id, goal_position, present_pos);
            bool is_hardware_error = dyn_comm_.Read(id, hardware_error_status); // ここでは雑に判定している．本来の返り値はuint8_tで各ビットに意味がある. 
